...
| `src/i18n.js` / `src/app.js` | 字典 + `t()`／瀏覽器 UI、hash 路由。我是誰的**世代/地區分池**（`genKeyOf`）住在 `app.js`，吃 `dex-national.json`：一般／Mega 依本體國家圖鑑編號 `ndex` 分段；**地區形態歸到該形態登場的世代/地區**（`REGION_GEN`：阿羅拉→g7、伽勒爾→g8、帕底亞→g9），而非本體編號所屬世代（如阿羅拉九尾 ndex 38 但歸第七世代阿羅拉，不歸關都）；**洗翠**（Legends Arceus，世代算 g8 但地區非伽勒爾）獨立成 `hisui` 桶；冠軍賽季池則改吃 `pokedex.json`（Champions 名單）；黑影效果為 CSS `filter: brightness(0)`，作答後揭曉。「速度線表」（`#/speedline` → `viewSpeedChart`/`buildSpeedTable`）入口在速度測驗 setup，依當前賽季把同速寶可夢排一列、橫向可捲。「圖鑑」（`#/dex` → `viewDex`）入口在我是誰 setup：兩組標籤（世代/地區 + 賽制）、屬性過濾（單選）、全彩縮圖牆，點縮圖浮出名字 5 秒（`whoPool` 同時供世代/地區與賽季池）|
